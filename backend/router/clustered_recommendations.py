...
# Simple recommendation models (without clustering)
class SimpleRecommendationRequest(BaseModel):
    """Request for simple top 30 recommendations without clustering"""
    query: str = Field(..., description="What you want to do/see", 
                      example="I want to explore ancient temples and cultural sites in Sri Lanka")
    interests: List[str] = Field(..., description="Your interests", 
                                example=["culture", "temples", "history"])
    budget_level: Optional[str] = Field("medium", description="Budget level")
    group_size: Optional[int] = Field(2, description="Group size")
    top_k: int = Field(30, description="Number of recommendations to return", le=50)

class SimpleRecommendationResponse(BaseModel):
    """Response with simple list of top recommendations"""
    query: str
    total_attractions: int
    attractions: List[AttractionInfo]
    processing_time_ms: float

@router.post("/recommendations", response_model=SimpleRecommendationResponse)
async def get_simple_recommendations(request: SimpleRecommendationRequest):
    """
    Get top 30 attraction recommendations without clustering or optimization
    
    This endpoint:
    1. Uses PEAR ranking to get personalized recommendations
    2. Returns attractions with accurate coordinates
    3. No clustering, no route optimization - just pure recommendations
    4. Perfect for simple recommendation lists or custom itinerary building
    """
    start_time = time.time()
    
    try:
        logger.info(f"Getting simple recommendations for query: '{request.query}'")
        ...
        # Get top recommendations using PEAR ranking
        ...
        logger.info(f"Found {len(top_attractions)} attractions")
        ...
        for rec in top_attractions:
            ...
            # Enrich with accurate coordinates
            ...
            attraction_info = AttractionInfo(
                ...
                visit_order=None  # No ordering for simple recommendations
            )
            enriched_attractions.append(attraction_info)
        
        processing_time = (time.time() - start_time) * 1000
        
        logger.info(f"Generated {len(enriched_attractions)} simple recommendations in {processing_time:.2f}ms")
